...
public class MissionManager {
  ...
  public boolean createMissionFromClient(MissionFromClient body, boolean recompute) {
    ...
    String missionCacheKey = body.id.toString();
    boolean managedMissionOk = true;
    Cache cache = cacheManager.getCache("createMission");
    if (cache.get(missionCacheKey) == null) {
      log.debug(LOG_PREFIX + "Imposto la cache {} con valore true", missionCacheKey);
      //FIXME: impostare una scadenza della cache, prima era a , "1mn"
      cache.put(missionCacheKey, true);
    } else {
  ...
  private Situation getSituation(LocalDateTime actualDate, MissionFromClient body, 
    ...
    if (actualDate.toLocalDate().isEqual(body.dataInizio.toLocalDate()) 
      ...
      situation.difference = (body.dataFine.getHour() 
          * DateTimeConstants.MINUTES_PER_HOUR + body.dataFine.getMinute()) 
          - (body.dataInizio.getHour() 
              * DateTimeConstants.MINUTES_PER_HOUR + body.dataInizio.getMinute());
      ...
      workMinutes = workInterval.to.getHour() * DateTimeConstants.MINUTES_PER_HOUR
          + workInterval.to.getMinute();
      missionMinutes = actualDate.getHour() * DateTimeConstants.MINUTES_PER_HOUR
          + actualDate.getMinute();  
      ...
      actualDate = actualDate.withHour(body.dataFine.getHour());
      actualDate = actualDate.withMinute(body.dataFine.getMinute());
      workMinutes = workInterval.from.getHour() * DateTimeConstants.MINUTES_PER_HOUR 
          + workInterval.from.getMinute();
      missionMinutes = actualDate.getHour() * DateTimeConstants.MINUTES_PER_HOUR
          + actualDate.getMinute(); 
  ...
  public boolean manageMissionFromClient(MissionFromClient body, boolean recompute) {
    ...
    recalculate(body, Optional.ofNullable(missions));
  ...
  private void recalculate(MissionFromClient body, Optional<List<Absence>> missions) {
    ...
    consistencyManager.updatePersonSituation(body.person.getId(), begin);
  }
  ...
  private LocalDateTime rightDate(LocalDate date, MissionFromClient body) {
    LocalDateTime actual = body.dataInizio.withHour(body.dataFine.getHour())
        .withMinute(body.dataFine.getMinute())
        .withSecond(body.dataFine.getSecond());
