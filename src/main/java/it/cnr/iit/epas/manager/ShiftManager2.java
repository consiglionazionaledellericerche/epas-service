...
import it.cnr.iit.epas.dao.PersonShiftDao;
import it.cnr.iit.epas.dao.PersonShiftDayDao;
import it.cnr.iit.epas.dao.PersonShiftDayInTroubleDao;
import it.cnr.iit.epas.dao.ShiftDao;
import it.cnr.iit.epas.dao.ShiftTypeDao;
...
import it.cnr.iit.epas.models.ShiftType.ShiftPeriod;
...
public class ShiftManager2 {
  ...
  private final PersonShiftDao personShiftDao;
  private final PersonShiftDayDao personShiftDayDao;
  private final PersonDayDao personDayDao;
  private final PersonShiftDayInTroubleDao personShiftDayInTroubleDao;
  ...
  ShiftManager2(PersonDayManager personDayManager, 
    ...
    this.personShiftDayInTroubleDao = personShiftDayInTroubleDao;
  ...
  public List<ShiftType> getUserActivities() {
    ...
    if (person != null) {
      ...
      if (currentUser.isSystemUser()) {
        activities.addAll(shiftTypeDao.findAll());
      }
    }
   ...
   *
   * @return ShiftTroubles.PERSON_IS_empty, ""
   */
  ...
  public void populatePersonShiftTable() {
    ...
    List<PersonCompetenceCodes> shiftPeople = competenceCodeDao
        .listByCodes(codeList, Optional.ofNullable(LocalDate.now()));
    shiftPeople.forEach(item -> {
      if (personShiftDayDao.getPersonShiftByPerson(item.getPerson(), LocalDate.now()) == null) {
        ...
        personShiftDao.persist(personShift);
      } else {
  ...
  public Optional<String> shiftPermitted(PersonShiftDay personShiftDay) {
    ...
    List<PersonShiftDay> list = personShiftDayDao
        .byTypeInPeriod(personShiftDay.getDate(), personShiftDay.getDate(),
            personShiftDay.getShiftType(), Optional.empty());
  ...
  public void setShiftTrouble(final PersonShiftDay shift, ShiftTroubles cause) {
    ...
    if (!shift.hasError(trouble.getCause())) {
      personShiftDayInTroubleDao.persist(trouble);
  ...
  public void fixShiftTrouble(final PersonShiftDay shift, ShiftTroubles cause) {
    ...
    Optional<PersonShiftDayInTrouble> psdit = shift.getTroubles().stream()
        .filter(trouble -> trouble.getCause() == cause).findFirst();

    if (psdit.isPresent()) {
      shift.getTroubles().remove(psdit.get());
      personShiftDayInTroubleDao.delete(psdit.get());
  ...
  public void checkShiftValid(PersonShiftDay personShiftDay) {
    ...
    personShiftDayDao.merge(personShiftDay);
  ...
  public List<Person> involvedShiftWorkers(ShiftType activity, LocalDate from, LocalDate to) {
    return personShiftDayDao.byTypeInPeriod(from, to, activity, Optional.empty())
  ...
  public int calculatePersonShiftCompetencesInPeriod(ShiftType activity, Person person,
    ...
    if (setting != null) {
      ...
      beforeDawn = new TimeInterval(
          LocalTime.of(0, 0), convertFromString(setting.getEndNightlyShift()));
    ...
    switch (type) {
      case daily:
        timeInterval = Optional.ofNullable(daily);
        timeInterval2 = Optional.<TimeInterval>empty();
        ...
        break;
      case nightly:
        timeInterval = Optional.ofNullable(night);
        timeInterval2 = Optional.ofNullable(beforeDawn);
        list = shifts.stream().filter(day -> { 
          return !personDayManager.isHoliday(day.getPersonShift().getPerson(), day.getDate(), 
              setting.isSaturdayHolidayShift());
        }).collect(Collectors.toList());
      ...
      case holiday:
        if (setting.isHolidayShiftInNightToo()) {
          timeInterval = Optional.of(new TimeInterval(LocalTime.of(0, 0), LocalTime.of(23, 59)));
          timeInterval2 = Optional.<TimeInterval>empty();
        } else {
          timeInterval = Optional.ofNullable(daily);
          timeInterval2 = Optional.<TimeInterval>empty();
  ...
  private LocalTime convertFromString(String str) {
    ...
    LocalTime time = LocalTime.of(Integer.valueOf(s[0]), Integer.valueOf(s[1]));
    return time;
  }
  ...
  public int getPersonResidualShiftCompetence(Person person, YearMonth yearMonth) {
    ...
    Competence lastShiftCompetence = competenceDao.getLastPersonCompetenceInYear(person,
        yearMonth.getYear(), yearMonth.getMonth().getValue(), shiftCode);
  ...
  public List<PersonShiftShiftType> shiftWorkers(ShiftType activity, LocalDate start,
      LocalDate end) {
    if (shiftTypeDao.isPersistent(activity) && start != null && end != null) {
  ...
  public void assignShiftCompetences(ShiftTypeMonth shiftTypeMonth) {
    ...
    final LocalDate monthBegin = shiftTypeMonth.getYearMonth().atDay(1);
  ...
  private void saveCompetence(Person person, ShiftTypeMonth shiftTypeMonth, 
    ...
    competenceDao.merge(newCompetence);
    //newCompetence.save();
  ...
  public void save(PersonShiftDay personShiftDay) {
    personShiftDayDao.persist(personShiftDay);
  ...
  public void delete(PersonShiftDay personShiftDay) {
    personShiftDayDao.delete(personShiftDay);
    recalculate(personShiftDay);
  }
  ...
  private void recalculate(PersonShiftDay personShiftDay) {
    ...
    if (shiftType != null) {
      ...
      // Ricalcoli sul turno
      if (personShiftDayDao.isPersistent(personShiftDay)) {
      ...
      if (newStatus.getShiftType() != null) {
        ...
        newStatus.setYearMonth(YearMonth.from(personShiftDay.getDate()));
        newStatus.setShiftType(shiftType);
      }
      shiftTypeMonthDao.merge(newStatus);
    }
  }
