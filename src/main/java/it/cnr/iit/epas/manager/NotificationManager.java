...
import it.cnr.iit.epas.helpers.TemplateExtensions;
...
import it.cnr.iit.epas.utils.Mail;
...
public class NotificationManager {
  ...
  private void notifyStamping(Stamping stamping, Crud operation) {
    ...
    final String message = 
        String.format(
            template, person.fullName(), DateTimeFormatter.ofPattern(DTF).format(stamping.getDate()),
    ...
    person.getOffice().getUsersRolesOffices().stream()
        ...
        .map(uro -> uro.getUser()).forEach(user -> {
          if (operation != Crud.DELETE) {
            ...
            .subject(NotificationSubject.STAMPING, stamping.getId()).create();
          } else {
  ...
  private void notifyAbsenceRequest(AbsenceRequest absenceRequest, Crud operation) {
    ...
    final String message =
        String.format(template, person.fullName(), absenceRequest.getId(),
            DateTimeFormatter.ofPattern(DF).format(absenceRequest.getStartAt()));
    ...
    if (absenceRequest.isFullyApproved()) {
      ...
      .subject(NotificationSubject.ABSENCE_REQUEST, absenceRequest.getId()).create();
      ...
      List<Absence> absence =
          absenceDao.findByPersonAndDate(absenceRequest.getPerson(), absenceRequest.startAtAsDate(),
              Optional.of(absenceRequest.endToAsDate()), Optional.empty()).list();
    ...
    if (roleDestination.name.equals(Role.GROUP_MANAGER)) {
      ...
      for (User user : users) {
        for (Group group : groups) {
          if (group.getManager().equals(user.person) && group.getPeople().contains(person)) {
            Notification.builder().destination(user).message(message)
            .subject(NotificationSubject.ABSENCE_REQUEST, absenceRequest.getId()).create();
          }
        }
  ...
  private Role getProperRole(AbsenceRequest absenceRequest) {
    ...
    if (absenceRequest.isAdministrativeApprovalRequired()
        ...
        && (absenceRequest.getManagerApproved() != null || !absenceRequest.isManagerApprovalRequired())) {
    ...
    if (absenceRequest.isOfficeHeadApprovalForManagerRequired()
        && absenceRequest.getOfficeHeadApproved() == null && absenceRequest.getPerson().isGroupManager()) {
      role = roleDao.getRoleByName(Role.SEAT_SUPERVISOR);
    }
    return role;
  }
  ...
  public void notificationStampingPolicy(User currentUser, Stamping stamping, boolean insert,
    ...
    if (secureManager.officesWriteAllowed(currentUser).contains(currentUser.person.getOffice())) {
      return;
    }

    // Se l'user che ha fatto l'inserimento è tecnologo e può autocertificare le timbrature esco
    if (currentUser.person.getOffice().checkConf(EpasParam.TR_AUTOCERTIFICATION, "true")
        && currentUser.person.getQualification().getQualification() <= 3) {
      return;
    }
    ...
    val config = configurationManager.configValue(currentUser.person.getOffice(),
  ...
  public void notificationAbsencePolicy(User currentUser, Absence absence,
    ...
    if (currentUser.person != null
        && secureManager.officesWriteAllowed(currentUser).contains(currentUser.person.getOffice())) {
      return;
    }
  ...
  public void notificationAbsenceRequestRefused(AbsenceRequest absenceRequest, Person refuser) {
    ...
    final String message = String.format(
        ...
        absenceRequest.getType().isAllDay() ? TemplateExtensions.format(absenceRequest.startAtAsDate())
            : TemplateExtensions.format(absenceRequest.getStartAt()),
            absenceRequest.getType().isAllDay() ? TemplateExtensions.format(absenceRequest.endToAsDate())
  ...
  public void notificationAffiliationRemoved(Set<Contract> contracts, Group group) {
    ...
    try {
      simpleEmail.addTo(userDestination.person.getEmail());
    ...
    log.info("Inviata email per rimozione utenti dal gruppo {} perchè contratto scaduto:"
        + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
        groupName, userDestination.person.getEmail(), simpleEmail.getSubject(), mailBody);
  }
  ...
  public void notificationShiftRemoved(Set<Contract> contracts,
    ...
    log.info("Inviata email per rimozione utenti dal turno {} perchè contratto scaduto:"
        + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
        shiftType, userDestination.person.getEmail(), simpleEmail.getSubject(), mailBody);
  }
  ...
  public void notificationReperibilityRemoved(Set<Contract> contracts,
    ...
    log.info("Inviata email per rimozione utenti dalla reperibilità {} perchè contratto scaduto:"
        + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
        description, userDestination.person.getEmail(), simpleEmail.getSubject(), mailBody);
  }
  ...
  private void sendEmailAbsenceRequest(AbsenceRequest absenceRequest) {
    ...
    person.getOffice().getUsersRolesOffices().stream().filter(uro -> uro.role.equals(roleDestination))
        .map(uro -> uro.getUser()).forEach(user -> {
          ...
          try {
            simpleEmail.addTo(user.person.getEmail());
          } catch (EmailException e) {
            e.printStackTrace();
          }
          simpleEmail.setSubject(String.format("ePas Approvazione flusso (%s)", absenceRequest.getId()));
          ...
          log.info(
              "Inviata email per richiesta di flusso richiesta: {}. "
                  + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
                  absenceRequest, user.person.getEmail(), simpleEmail.getSubject(), mailBody);
        });
  }
  ...
  public void sendEmailAbsenceNotification(AbsenceRequest absenceRequest) {
    ...
    if (configurationManager.configValue(absenceRequest.getPerson().getOffice(),
      ...
      recipients = 
          ...
          .filter(uro -> uro.role.name.equals(Role.SEAT_SUPERVISOR)).map(uro -> uro.getUser().person)
      ...
      recipients.forEach(r -> {
        try {
          ...
          simpleEmail.setSubject(
              String.format("ePas Comunicazione assenza (id=%s)", absenceRequest.getId()));
          ...
          log.info(
              "Inviata email per richiesta di flusso richiesta: {}. "
                  + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
                  absenceRequest, r.getUser().person.getEmail(), simpleEmail.getSubject(), mailBody);
        } catch (EmailException e) {
  ...
  public void notificationCompetenceRequestRefused(
    ...
    .subject(NotificationSubject.COMPETENCE_REQUEST, competenceRequest.getId()).create();
  ...
  private void notifyCompetence(Competence competence, User currentUser, Crud operation) {
    ...
    YearMonth yearMonth = YearMonth.of(competence.year, competence.month);
    final String message = String.format(template, modifier,
        DateTimeFormatter.ofPattern(DF).format(yearMonth), competence.competenceCode.code);
  ...
  private void sendEmailCompetenceRequest(CompetenceRequest competenceRequest) {
    ...
    log.info("Inviata email per richiesta di flusso richiesta: {}. "
        + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
        competenceRequest, userDestination.person.getEmail(), simpleEmail.getSubject(), mailBody);
  ...
  private void notifyInformationRequest(InformationRequest informationRequest, Crud operation) {
    ...
    if (informationRequest.isFullyApproved()) {
      Notification.builder().destination(person.getUser()).message(message)
      .subject(subject, informationRequest.getId()).create();
      // ...e all'amministratore del personale
    }
    ...
    users.forEach(user -> {
      Notification.builder().destination(user).message(message)
      .subject(notificationSubject, informationRequest.getId()).create();
    });
  }
  ...
  public void notificationInformationRequestRefused(Optional<ServiceRequest> serviceRequest,
    ...
    .subject(notificationSubject, request.getId()).create();
    ...
    if (roleDestination.equals(roleDao.getRoleByName(Role.GROUP_MANAGER))) {
      person.getAffiliations().stream().map(gp -> gp.getGroup().getManager().getUser()).forEach(user -> {
        ...
        log.info(
            "Inviata email per richiesta di flusso richiesta: {}. "
                + "Mail: \n\tTo: {}\n\tSubject: {}\n\tbody: {}",
                informationRequest, user.person.getEmail(), simpleEmail.getSubject(), mailBody);
      });
    } else {
      person.getOffice().getUsersRolesOffices().stream().filter(uro -> uro.role.equals(roleDestination))
          .map(uro -> uro.getUser()).forEach(user -> {
  ...
  private String createInformationRequestEmail(InformationRequest informationRequest, User user) {
    ...
    switch (informationRequest.getInformationType()) {
      case ILLNESS_INFORMATION:
        IllnessRequest illnessRequest = requestDao.getIllnessById(informationRequest.getId()).get();
        ...
        break;
      case SERVICE_INFORMATION:
        ServiceRequest serviceRequest = requestDao.getServiceById(informationRequest.getId()).get();
    ...
    baseUrl = baseUrl + INFORMATION_PATH + "?id=" + informationRequest.getId() + "&type="
    ...
    if (Strings.isNullOrEmpty(replayTo)) {
      person.getOffice().getUsersRolesOffices().stream()
          ...
          .map(uro -> uro.getUser()).forEach(u -> {
            try {
              simpleEmail.addCc(u.person.getEmail());
            } catch (EmailException e) {
