...
public class AbsenceTypeDao extends DaoBase<AbsenceType> {
  ...
  @Inject
  AbsenceTypeDao(ObjectProvider<EntityManager> emp) {
    super(emp);
  }

  public List<AbsenceType> findAll() {
    return getQueryFactory().selectFrom(QAbsenceType.absenceType).fetch();
  }
  ...
  public Map<AbsenceType, Long> getAbsenceTypeInPeriod(
    ...
    return getQueryFactory().from(absenceType)
        .join(absenceType.absences, absence).where(absence.personDay.person.eq(person).and(
            absence.personDay.date.between(fromDate, toDate.orElse(fromDate))))
        .groupBy(absenceType)
        .orderBy(absence.count().desc())
        .transform(groupBy(absenceType).as(absence.count()));
  }
