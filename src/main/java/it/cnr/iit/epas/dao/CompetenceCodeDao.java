...
import it.cnr.iit.epas.models.QCompetenceCodeGroup;
import it.cnr.iit.epas.models.QPersonCompetenceCodes;
import it.cnr.iit.epas.models.dto.PersonCompetenceCodeDto;
...
public class CompetenceCodeDao extends DaoBase<CompetenceCode> {
  ...
  @Inject
  CompetenceCodeDao(ObjectProvider<EntityManager> emp) {
    super(emp);
  }
  ...
  public List<PersonCompetenceCodes> listByPerson(Person person, Optional<LocalDate> date) {
    ...
    if (date.isPresent()) {
      condition.and(pcc.beginDate.loe(DateUtility.endOfMonth(date.get()))
          .andAnyOf(pcc.endDate.isNull(),
              pcc.endDate.goe(DateUtility.endOfMonth(date.get()))));
